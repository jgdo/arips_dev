...
namespace tf2_msgs
{
  ...
  {
    public:
      char * target_frame = nullptr;
      char * source_frame = nullptr;
      ros::Time source_time;
      ros::Duration timeout;
      ros::Time target_time;
      char * fixed_frame = nullptr;
      bool advanced = 0;
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      int offset = 0;
      {
      uint32_t length_ = strlen( (const char*) this->target_frame);
      memcpy(outbuffer + offset, &length_, sizeof(uint32_t));
      offset += 4;
      memcpy(outbuffer + offset, this->target_frame, length_);
      offset += length_;
      }
      {
      uint32_t length_ = strlen( (const char*) this->source_frame);
      memcpy(outbuffer + offset, &length_, sizeof(uint32_t));
      offset += 4;
      memcpy(outbuffer + offset, this->source_frame, length_);
      offset += length_;
      }
      ...
      *(outbuffer + offset + 3) = (this->target_time.nsec >> (8 * 3)) & 0xFF;
      ...
      {
      uint32_t length_ = strlen( (const char*) this->fixed_frame);
      memcpy(outbuffer + offset, &length_, sizeof(uint32_t));
      offset += 4;
      memcpy(outbuffer + offset, this->fixed_frame, length_);
      offset += length_;
      }
      {
      union {
