  ...
  class MenuEntry : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 3) = (this->parent_id >> (8 * 3)) & 0xFF;
      ...
      {
      uint32_t length_ = strlen( (const char*) this->title);
      memcpy(outbuffer + offset, &length_, sizeof(uint32_t));
      offset += 4;
      memcpy(outbuffer + offset, this->title, length_);
      offset += length_;
      }
      {
      uint32_t length_ = strlen( (const char*) this->command);
      memcpy(outbuffer + offset, &length_, sizeof(uint32_t));
      offset += 4;
      memcpy(outbuffer + offset, this->command, length_);
      offset += length_;
      }
