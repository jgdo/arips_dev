...
namespace brics_actuator
{
  ...
  class CartesianWrench : public ros::Msg
  {
    public:
      ...
      char * base_frame_uri = nullptr;
      char * target_frame_uri = nullptr;
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 3) = (this->timeStamp.nsec >> (8 * 3)) & 0xFF;
      ...
      {
      uint32_t length_ = strlen( (const char*) this->base_frame_uri);
      memcpy(outbuffer + offset, &length_, sizeof(uint32_t));
      offset += 4;
      memcpy(outbuffer + offset, this->base_frame_uri, length_);
      offset += length_;
      }
      {
      uint32_t length_ = strlen( (const char*) this->target_frame_uri);
      memcpy(outbuffer + offset, &length_, sizeof(uint32_t));
      offset += 4;
      memcpy(outbuffer + offset, this->target_frame_uri, length_);
      offset += length_;
      }
